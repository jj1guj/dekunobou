...
// パスの判定
bool Board::is_pass() {
  unsigned long long playerLegalBoard =
      makeLegalBoard(board_player, board_opponent);
  unsigned long long opponentLegalBoard =
      makeLegalBoard(board_opponent, board_player);

  return (playerLegalBoard == 0x0000000000000000) &&
         (opponentLegalBoard != 0x0000000000000000);
}
